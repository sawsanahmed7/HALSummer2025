...
class Cell3 extends AgentSQ2Dunstackable<CAR_T> {
    ...
    public void Step() {
        if (this.mutated) {
            ...
            if (G.rn.Double() < G.BIRTH_PROB) {
                int nOptions = G.MapEmptyHood(G.mooreHood, Xsq(), Ysq());
                if (nOptions > 0) {
                   if (G.rn.Int(mutationLimit) == 1 && !carT) {
...
class CAR_T_cell extends Cell3 {

    @Override
    public void Step() {
        this.carT = true;

        int neighborhood = G.MapOccupiedHood(G.mooreHood, Xsq(), Ysq());
        System.out.println("hii");
        System.out.println(neighborhood);


        if (G.rn.Double() < (G.DEATH_PROB/(G.rn.Double()))) {
            CAR_T.removeColor(this.colorRef);
            //System.out.println("mutated [" + this.colorRef + "] cell died");
            Dispose();
            return;
        }
        if (G.rn.Double() < (G.BIRTH_PROB*(G.rn.Double()))) {
            int nOptions = G.MapEmptyHood(G.mooreHood, Xsq(), Ysq());
            if (nOptions > 0) {
                CAR_T_cell c1 = (CAR_T_cell) G.NewAgentSQ(G.mooreHood[G.rn.Int(nOptions)]);
                c1.color = this.color;
                c1.colorRef = this.colorRef;
                c1.mutated = this.mutated;
                CAR_T.addColor(this.colorRef);
...
public class CAR_T extends AgentGrid2D<Cell3> {
    ...
    int color;
    public CAR_T(int x, int y, int color) {
    ...
    public void CT_setup(double rad){
        color = MAGENTA;
        int[] coords = CircleHood(true, rad);
        int nCoords = MapHood(coords, 25, 25);
        for (int i = 0; i < nCoords; i++) {
            CAR_T_cell ct_cell = (CAR_T_cell) NewAgentSQ(coords[i]);
            ct_cell.color = color;
            ct += 1;
        }
